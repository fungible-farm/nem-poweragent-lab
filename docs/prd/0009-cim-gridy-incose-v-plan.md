...
|---|---|---|---|
| ~~UFO endurant/perdurant/moment stereotypes (4-enum split)~~ | ~~`ledgrrr` (`crates/ufo-types`)~~ **Resolved 2026-08-24** — see below | Already shipped, tested, used internally by `ledger-core`'s `Stereotyped for ArtifactKind` *(pre-reconciliation state — `ledger-core` never actually shipped a real `ArtifactKind` stereotyping; the one real attempt, #145, is stranded on the unmerged `b00t-patches` branch, confirmed zero consumers of these 4 types on `ledgrrr` `main`)* | High |
| `UfoStereotype` (single enum: Kind/SubKind/Role/Relator/Mode) + `dare.rs` (Decision/Alternative/Risk/ExecutiveDecision/OodaStateMachine) | standalone `promptexecution/ufo-types` | cim-gridy's Phase 3 optimizer directly depends on `dare.rs`, which does not exist in `ledgrrr`'s copy; this is the real, live dependency, not the vendored one — **unchanged by the reconciliation below**, still the crate cim-gridy pins | High |
| Reconciling the two `ufo-types` implementations | **Resolved 2026-08-24** — standalone `promptexecution/ufo-types` (single source of truth) | `UfoStereotype` gained `ledgrrr`'s missing endurant/perdurant substereotypes (`Phase`/`Category`/`Mixin`/`RoleMixin`/`Process`/`State`/`Event`/`Scenario`/`Abstract`) plus a `.category()` method — existing variants and `Display` output unchanged, zero source change for `dare.rs`/`capability.rs`/cim-gridy/`b00t-cli`. `satisfies`/`iso` adopted `ledgrrr`'s exact `SatisfiesResult`/`Disposition`/`NodeId`/`Lei`/`Currency`/`BankAccount`/`FinancialInstrument` shape (the one with ~60 real `ledger-core`/`ledgerr-mcp` call sites) — a source-path change for that consumer, not a behavior change (201+44 tests passing, identical to pre-migration baseline). `sysml.rs` (missed in the original MECE pass — its `holon-viz`/`sysml-derive` dependents are dev-dependency-only, so it didn't show up in a `[dependencies]` grep) ported too, opt-in `sysml` feature. `ledgrrr`'s vendored `crates/ufo-types` deleted; `ledger-core`/`ledgerr-mcp`/`holon-viz`/`sysml-derive` repointed at the external crate — completing #145's original, abandoned intent. See [`promptexecution/ufo-types#3`](https://github.com/PromptExecution/ufo-types/pull/3) and [`ledgrrr#201`](https://github.com/PromptExecution/ledgrrr/pull/201) (both open, unmerged as of this writing) | High |
...
two `ufo-types` implementations~~ **done 2026-08-24, see the MECE table above**; any new code
touching `mission-engine` or `ledgrrr` source *(the `ufo-types` reconciliation above is the one
exception — it does touch `ledgrrr` source, done deliberately once the design was concrete rather
than left as an unresolved MECE row)*; ~~the
