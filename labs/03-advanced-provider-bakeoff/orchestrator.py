...
SCORECARD_FILE = RESULTS_DIR / "scorecard.json"  # gitignored, regenerated by
# every report_step() call -- not committed. wall_clock_s differs every real
# run, which made a committed copy pure git churn with no diffable value;
# the actual committed, diffable fixture is EXPECTED_FILE below.
EXPECTED_FILE = LAB_DIR / "expected_scorecard.json"
SCORECARD_CHART_FILE = RESULTS_DIR / "scorecard_chart.png"  # also gitignored,
# also regenerated by every report_step() call. Note the churn rationale here
# differs from SCORECARD_FILE's: the chart plots only error_margin, which is
# deterministic (it's the fixture-diffed quantity), so a committed PNG would
# NOT churn on re-derivation -- the risk is cross-machine matplotlib-version
# rendering differences, and committing it would make check_step() assert a
# file a fresh clone doesn't have. Same "never commit regenerated samples"
# policy either way (see docs/backlog/0003-lab3-scorecard-visualization.md).
# Lives in RESULTS_DIR (not LAB_DIR)
# because it is a rendering of SCORECARD_FILE, which itself already lives
# there -- keeping the chart next to the data it renders, same locality
# principle as Lab 4's CHART_FILE sitting next to EXPECTED_FILE in LAB_DIR.
...
def report_step(rows: Optional[list[TaskFamilyResult]] = None) -> list[TaskFamilyResult]:
    ...
    print it as a table, and (re)render the scorecard chart PNG.

    Neither SCORECARD_FILE nor SCORECARD_CHART_FILE is committed (see their
    own comments) -- both are regenerated, local-only output, unconditional
    on every report_step() call, unlike Lab 4/5's `refresh_chart`-gated
    pattern (that gate exists specifically to protect a *committed* fixture
    from being clobbered by an ad-hoc run; there is nothing committed here
    to protect). check_step() below calls sweep_step() directly and never
    calls report_step(), so an ordinary `--step check` never touches either
    file.
...
def check_step() -> bool:
    ...
    Does not assert anything about SCORECARD_CHART_FILE: neither it nor
    SCORECARD_FILE is committed (both are regenerated, local-only output,
    see their own comments), so their presence on a given checkout says
    nothing about correctness -- a fresh clone legitimately has neither
    until `--step report` is run once. The chart's real "does it still
    render" check is exercising report_step() directly (see test_lab3.py),
    not check_step().
