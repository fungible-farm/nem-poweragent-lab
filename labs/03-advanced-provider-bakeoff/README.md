...
4. Run the same matrix as a Kubernetes `Job` (`kube/benchmark-runner-job.yaml`) so the 3×N sweep
   can be farmed out as parallel pods instead of a serial loop.
5. Write a single scorecard (JSON + printed table) to
...
Operations-relevant call, because the workload is genuinely parallel and genuinely repeated.
